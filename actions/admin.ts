...
export async function createProduct(
  ...
  try {
    ...
    const colors = parseJsonField<string[]>(formData.get("colors"), []).filter(Boolean);
    const sizes = parseJsonField<string[]>(formData.get("sizes"), []).filter(Boolean);
...
export async function updateProduct(
  ...
  try {
    ...
    for (const [key, value] of formData.entries()) {
      if (key !== "images" && key !== "videos" && value !== "") rawData[key] = value;
    }
    const images = formData.getAll("images").filter(Boolean).map(String);
    if (images.length > 0) rawData.images = images;
    const videos = formData.getAll("videos").filter(Boolean).map(String);
    rawData.videos = videos;
    rawData.colors = parseJsonField<string[]>(formData.get("colors"), []).filter(Boolean);
    rawData.sizes = parseJsonField<string[]>(formData.get("sizes"), []).filter(Boolean);
    rawData.customFields = parseJsonField(formData.get("customFields"), []);

    if (rawData.originalCost) rawData.originalCost = Number(rawData.originalCost);
    if (rawData.markupPrice) rawData.markupPrice = Number(rawData.markupPrice);
    if (rawData.moq) rawData.moq = Number(rawData.moq);
    if (rawData.priceLockDays) rawData.priceLockDays = Number(rawData.priceLockDays);
    if (rawData.isPreorder) rawData.isPreorder = rawData.isPreorder === "true";
    if (formData.has("categoryId") && !formData.get("categoryId")) rawData.categoryId = null;
...
export async function updateOrderStatus(
  ...
  try {
    ...
    const notificationMap: Record<string, { type: "ITEM_PROCURED" | "OUT_FOR_DELIVERY" | "DELIVERED"; message: string }> = {
      PROCURED: {
        type: "ITEM_PROCURED",
