...
import { AdminProductsFilter } from "@/components/shared/admin-products-filter";
...
export default async function AdminProductsPage({
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
      ...
      <AdminProductsFilter q={q} status={status} />

      <div className="rounded-2xl border border-border/60 overflow-x-auto">
        <table className="w-full min-w-[560px] text-sm">
          <thead>
            <tr className="border-b border-border/40 bg-muted/30">
              <th className="px-5 py-3.5 text-left text-xs tracking-[0.1em] uppercase font-medium text-muted-foreground">
                Name
              </th>
              <th className="px-5 py-3.5 text-left text-xs tracking-[0.1em] uppercase font-medium text-muted-foreground">
                Original Cost
